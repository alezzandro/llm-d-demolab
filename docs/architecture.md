...
│                                                                            │
│     Gen AI Playground (OGX → vLLM HTTPS ClusterIP, bypasses MaaS)          │
...
| `MaaSModelRef` `llama-3-1-8b-fp8` | Registers pool with MaaS (name = inference URL path) | `models-as-a-service` |
...
| `OGXServer` `ogx-genai-playground` | Gen AI Playground (CPU; remote vLLM over HTTPS ClusterIP) | `models-as-a-service` |
| PostgreSQL `ogx-postgres` | KV/SQL store for the OGX `rh` distribution | `models-as-a-service` |
| `MCPServer` `openshift-mcp-server` | Read-only OpenShift MCP for Playground | `models-as-a-service` |
...
- AutoML / training jobs (would compete with the 4× L4 llm-d pool)

Phase 4 enables OpenShift AI 3.5 **dashboard flags** and **control-plane** DSC operators (OGX, AI Pipelines, TrustyAI, MLflow) so Gen AI Studio, MCP catalog, Eval Hub, and llm-d templates appear in the UI. Ray, Kueue, and TrainingOperator stay **Removed**. Phase 6 deploys a CPU `OGXServer` (`rh`), dedicated PostgreSQL (`ogx-postgres`), and the OpenShift MCP server so **Gen AI Studio → Playground** can chat against the llm-d pool over **HTTPS** ClusterIP (bypasses MaaS, same as the original MaaS demo). See [Activating the OGX Operator](https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.5/html/working_with_ogx/activating-the-ogx-operator_rag), [Deploying a OGX server](https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.5/html/working_with_ogx/deploying-ogx-server_rag), and [Experimenting with models in the gen AI playground](https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.5/html/experimenting_with_models_in_the_gen_ai_playground/index).
...
2. **MaaS vs ODH AuthPolicy** — The Gateway must be annotated `opendatahub.io/managed: "false"` so `odh-model-controller` does not install `{gateway}-authn` on the same Gateway (that would override MaaS `maas-gateway-auth`). The `LLMInferenceService` must set `security.opendatahub.io/enable-auth: "true"`. `enable-auth: "false"` makes the controller install anonymous `{llmis}-kserve-route-authn` on the HTTPRoute; Kuadrant then drops `auth.identity`, TokenRateLimitPolicy CEL fails (`NoSuchKey("identity")`), Limitador never increments, and **Observability → Usage** stays at 0 with empty subscription filters. Phase 8 and `health-check.sh --fix` delete leftover route/gateway AuthPolicies. See [Govern LLM access with Models-as-a-Service (3.5)](https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.5/html/govern_llm_access_with_models-as-a-service/) and [MaaS conflicting AuthPolicy troubleshooting](https://github.com/opendatahub-io/models-as-a-service/blob/main/docs/content/install/troubleshooting.md).
...
12. **Dev Spaces Continue extension** — `postStart` copies `continue-ai-config` to `~/.continue/` (json + yaml for Continue 2.x) and downloads `Continue.continue.vsix` onto persistUserHome. Che-code opens `/projects/.code-workspace`, so sample-folder recommendations never install. Phase 9 applies two ConfigMaps into every user workspace namespace: `vscode-editor-configurations` (recommendations) and `vscode-default-extensions` (`DEFAULT_EXTENSIONS` vsix install). Do **not** put Open VSX item HTML URLs in CheCluster `defaultPlugins` — Che fetches those URIs as plugin YAML and the factory fails (`mapping values are not allowed in this context`). See [Configuring default extensions](https://eclipse.dev/che/docs/stable/administration-guide/default-extensions-for-microsoft-visual-studio-code/) and [Configuring Visual Studio Code](https://docs.redhat.com/en/documentation/red_hat_openshift_dev_spaces/3.24/html/administration_guide/configuring-visual-studio-code). Restart the DevWorkspace after those ConfigMaps land; the vsix install can take ~60s after the editor process starts.
13. **Usage dashboard (subscription filters / counters at 0)** — Perses queries Limitador `authorized_hits_total` / `authorized_calls_total`. Those series exist only after TokenRateLimitPolicy sees `auth.identity.selected_subscription_key`. Two booth-specific breaks: (a) competing `*-kserve-route-authn` (item 2), (b) `MaaSModelRef` named differently from the client URL (`llama-3-1-8b` vs `/models-as-a-service/llama-3-1-8b-fp8/`). The ref, subscriptions, and auth policies must use `llama-3-1-8b-fp8`. After a chat, scrape is ~30s. See [Limitador metrics](https://docs.kuadrant.io/latest/kuadrant-operator/doc/user-guides/observability/limitador-metrics/).
14. **Gen AI Playground (LlamaStack → OGX)** — OpenShift AI 3.5 replaced `LlamaStackDistribution` with `OGXServer` (`ogx.io/v1beta1`). The `rh` distribution requires PostgreSQL (`POSTGRES_*`); do not reuse the MaaS Postgres in `redhat-ods-applications`. Remote vLLM must be `https://…-kserve-workload-svc:8000/v1` with `VLLM_TLS_VERIFY=false` (workload Service `appProtocol=https`). Do not set `spec.providers.inference.remote.vllm` on the CR — the operator then fetches OCI labels from `registry.redhat.io` and fails HTTP 401 (`ConfigGenerationFailed`). Env-only `VLLM_URL` / `INFERENCE_MODEL` is the working inference path. The `rh` image **strips** `inline::sentence-transformers` and `inline::faiss` from default `config.yaml`; Playground `POST /v1/vector_stores` then 400s (`Model 'sentence-transformers/ibm-granite/granite-embedding-125m-english' not found`). Phase 6 applies `overrideConfig` ConfigMap `ogx-playground-config` (label `ogx.io/watch=true`) plus `HF_HOME=/opt/app-root/.cache/huggingface` so the baked Granite embedding weights are used. Playground traffic bypasses MaaS and will not appear on Usage.
