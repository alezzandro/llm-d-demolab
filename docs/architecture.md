...
8. **Bootstrapper** — `git config --global --add safe.directory /work/src` because the PVC clone is not owned by the container user. `oc logs` only follows PID 1; after a failure the entrypoint sleeps — resume with `DEMO_RETRY=true` and delete the pod (see [bootstrapper.md](bootstrapper.md)).
